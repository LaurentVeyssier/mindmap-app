...
│   │   ├── auth.py         # JWT tokens & native bcrypt password hashing
...
└── frontend/
    ...
    │   │   └── Login.tsx           # Premium glassmorphic authentication portal
    │   ├── App.tsx
    │   ├── App.css
    │   ├── main.tsx
    │   └── index.css
...
*   `app/config.py`: Loads and parses environment configuration using Pydantic BaseSettings. Validates Neo4j credentials, LLM API keys, and JWT parameters.
*   `app/logger.py`: Sets up a customized logger with Rich standard logging handler to present color-coded console logs.
*   `app/auth.py`: Implements password hashing functions using native `bcrypt`, access token generation with expiration (7 days), and FastAPI current user extraction dependency handlers.
*   `app/schemas.py`: Houses data models representing Neo4j graph nodes and edges, API payloads, auth requests (registration, token), and expected Gemini Pydantic schemas.
*   `app/neo4j_client.py`: Provides transaction methods to safely write, update, clear, and traverse graphs scoped to the authenticated user ID, manage users (fetch, create), and retrieve other graph nodes for negative space boundaries.
