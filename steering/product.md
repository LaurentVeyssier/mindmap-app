...
5.  **Interactive Graph Visualization**: Fully zoomable, panable, and draggable interactive canvas powered by D3 force-directed simulations (`react-force-graph-2d`).
...
14. **Dynamic Screen Responsiveness**: Supports fully responsive screen sizes, shifting the desktop sidebar to an auto-stacking vertical configuration, scrolling breadcrumbs horizontally, wrapping long text nodes, and omitting link relationship label noise on mobile viewports.
15. **User Session Authentication & Data Scoping**: Secure email/password account registration and login session validation. Restricts mindmap list, graph generation, drill-down, and article creation to the active logged-in user so that each user has private, separate workspaces.
