...
class TestDeleteTopic(unittest.TestCase):
    ...
    def test_delete_mindmap_endpoint_success(self, mock_neo4j_client: MagicMock) -> None:
        ...
        response = delete_mindmap(topic_id="test-topic-id")

        # Assertions
        mock_neo4j_client.get_topic.assert_called_once_with("test-topic-id")
        mock_neo4j_client.delete_topic.assert_called_once_with("test-topic-id")
    ...
    def test_delete_mindmap_endpoint_not_found(self, mock_neo4j_client: MagicMock) -> None:
        ...
        with self.assertRaises(HTTPException) as context:
            delete_mindmap(topic_id="non-existent")
        
        self.assertEqual(context.exception.status_code, 404)
        self.assertEqual(context.exception.detail, "Topic not found")
        mock_neo4j_client.get_topic.assert_called_once_with("non-existent")
